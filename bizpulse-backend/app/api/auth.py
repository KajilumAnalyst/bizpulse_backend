...
from app.schemas.schemas import RegisterRequest, LoginRequest, TokenResponse, UserOut, UserUpdate
...
def register(body: RegisterRequest, db: Session = Depends(get_db)):
    ...
    default_store = Store(owner_id=user.id, name=f"{body.business_name} — Main", type=body.business_type)
    db.add(default_store)
    db.commit()
    db.refresh(user)

    token = create_access_token({"sub": str(user.id)})
    return TokenResponse(access_token=token, user_id=user.id, business_name=user.business_name, plan=user.plan)
